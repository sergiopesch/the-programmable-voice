...
- Capable desktop browsers may open a native-4K Three.js hardback (`src/lib/book3d/`, `src/components/Book3DStage.tsx`), supersampled to 8K when the GPU allows. The cover arrives and opens as a physical artefact; section changes use a developable travelling leaf (`src/lib/pageturn/`). Semantic HTML remains the manuscript; WebGL is enhancement with reduced-motion, forced-colors and context-loss fallbacks.
