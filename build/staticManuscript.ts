...
import { sectionMarker, sectionPosition } from '../src/lib/book'
...
export function renderStaticManuscript() {
  const contents = sections.map((section) => `<li><a href="#${escapeHtml(section.id)}"><span>${sectionMarker(section)}</span>${escapeHtml(section.title)}</a></li>`).join('')
  const manuscript = sections.map((section) => `
    <article id="${escapeHtml(section.id)}">
      <header>
        <span>${escapeHtml(section.part)} · ${escapeHtml(sectionPosition(section))}</span>
...
<body>
  <header class="masthead"><span>Static reading edition</span><h1>The Programmable Voice</h1><p>A material history of sound, music and the human voice—from vibrating air to machines that listen and answer.</p><p>This edition contains the complete manuscript and evidence register without JavaScript, animation, or audio.</p></header>
